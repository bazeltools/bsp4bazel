...
def _bsp_metadata_impl(ctx):
    ...
    compiler_jars = ctx.attr._scala_compiler_toolchain[JavaInfo].compile_jars.to_list()
    semanticdb_jars = ctx.attr._semanticdb_scalac_plugin[JavaInfo].compile_jars.to_list()
...
"""
    ...
    ctx.actions.write(
        ...
        json_tpl.format(
            ...
            [j.path for j in compiler_jars], 
            [j.path for j in semanticdb_jars],
            paths, 
            packages, 
    ...
    return DefaultInfo(
        runfiles = ctx.runfiles(files = [ctx.outputs.bsp_target] + compiler_jars + semanticdb_jars),
    )
...
bsp_metadata = rule(
    ...
    attrs = {
        ...
        "_scala_compiler_toolchain": attr.label(
            default = Label("@io_bazel_rules_scala//scala/private/toolchain_deps:scala_compile_classpath"),
            allow_files = False,
        ),
        "_semanticdb_scalac_plugin": attr.label(
            default = Label("@io_bazel_rules_scala//scala/private/toolchain_deps:semanticdb_scalac")
        ),
        "_semanticdb": attr.label(
            default = Label("@io_bazel_rules_scala//scala/private/toolchain_deps:scala_compile_classpath"),
            allow_files = False,
        ),
    },
...
bsp_compile_and_gather = rule(
    ...
    attrs = {
        "_template": attr.label(
            default = Label("@bazel-bsp-rules//private:unzip_jar_metainf.sh.tpl"),
